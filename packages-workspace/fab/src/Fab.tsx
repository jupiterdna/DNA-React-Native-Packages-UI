...
import { fabSizeCls, getBGColor, styles } from "./styles";
  ...
  (
    ...
    const animateItems = useCallback((): void => {
      items?.forEach((_, index) => {
        ...
        animatedValues &&
          (animatedValues[indexVal].value = withDelay(
            index * ANIM_DURATION,
     ...
     */
    const _renderChildItems = useCallback(
     ...
     */
    const _renderMainFab = useCallback(
     ...
     */
    const _renderRefItems = useCallback(
      (): React.JSX.Element | null => (
    ...
    const _renderModal = useCallback((): React.JSX.Element | null => {
      ...
      return (
        ...
        >
          <Pressable
            style={[styles.modalContainer, getBGColor(scheme)]}
            onPress={(event) => handlePress(event)}
          />
          <TouchableOpacity
            style={[
